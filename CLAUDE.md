...
**Talos gotcha**: `machine.nodeTaints` does not work, and the failure is silent from the cluster side. Taints are applied by patching the Node after it registers, and the **NodeRestriction** admission plugin — on by default in Talos, not disableable — refuses to let a kubelet modify taints on its own Node (`siderolabs/talos#13443`). The controller retries forever in the node's own log and the Node comes up untainted. The PIA pool therefore uses `machine.kubelet.extraConfig.registerWithTaints`, which sets the taint at Node *creation*, where NodeRestriction permits it. This only bites when a Node object is **recreated** — a plain reboot keeps existing taints — but that is exactly what a podCIDR re-registration does, and an untainted PIA node also loses its `node-type` label, so its own workloads can no longer schedule on it. If you find a re-registered node bare, `kubectl label` and `kubectl taint` it by hand *immediately*, before anything schedules there.

### CNI: Cilium

- Routing mode: VXLAN tunnel
- **Kubernetes is dual-stack; Cilium is not — see the blocker in Key Gotchas.** `clusterPodNets` is `10.52.0.0/16` + `2a0e:cb01:ca:fe80::/60` and `clusterSvcNets` is `10.53.0.0/16` + `fd53:0:0:53::/112`, so the apiserver and controller-manager run dual-stack and a re-registered node gets a v6 podCIDR. Cilium keeps `enable-ipv6: "false"`, so none of it is used yet. The v6 pod range is a **GUA** on purpose: the goal is `enable-ipv6-masquerade: "false"`, letting Tailscale proxy pods advertise a direct endpoint instead of relaying every session. Nothing here is load-bearing while Cilium has IPv6 off
- IPv4 must be listed first in `clusterPodNets`, `clusterSvcNets` *and* `machine.kubelet.nodeIP.validSubnets` — Talos fails config parsing if the family order differs between them
...
- `coredns-custom` ConfigMap in `kube-system` blocks AAAA responses, so workloads resolve v4 only — correct while Cilium has IPv6 off, and deliberate afterwards too. See the Key Gotchas entry
...
- **`talos/manifests/cilium.yaml` is generated; `cilium-values.yaml` is the input.** Re-render with the `helm template` command in `kubernetes/cilium/README.md` — never hand-edit the manifest. Hand-edits are how the Talos-specific settings (`MTU: 1450`, `kubeProxyReplacement`, `cgroup.hostRoot`, the reduced capability set, `k8sServiceHost`) went missing from the values file for months, leaving a documented `helm upgrade` command that would have broken the datapath had a release existed for it to act on. The values file is verified to reproduce the manifest exactly; keep it that way. `extraManifests` apply only at bootstrap, so re-rendering changes nothing live — updates reach a running cluster by applying the changed documents by hand.
- **Cilium IPv6 needs Cilium >= 1.20 on Talos 1.13, and the error message misdirects.** On Cilium 1.19 setting `enable-ipv6: "true"` made the agent fail its start hook with *"IPv6 is enabled, but the needed ip6tables tables are unavailable ... installing ip6tables and kernel modules"* and crashloop, taking that node's CNI with it. **Do not chase the modules.** Talos reports them `module not found` because they are compiled in, not loadable, and adding them to `machine.kernel.modules` only leaves `KernelModuleSpecController` failing on a loop. The real cause is [siderolabs/talos#13268](https://github.com/siderolabs/talos/discussions/13268): Talos 1.13 stripped tailscale from the `iptables` binaries, so the node falls back to native nft and Cilium before 1.20 could not do native nft. Fixed by the 1.20.1 upgrade; the constraint is worth remembering because a Cilium downgrade would silently reintroduce it.
- **`cilium_unreachable_nodes` sits at a steady ~4.5 and always has.** Node-to-node *direct IP* health probes do not cross the VLAN boundaries — a node reaches only its own VLAN's peers — so `cilium-health status` shows `Node 0/1` for remote-VLAN nodes while `Endpoints` stays `1/1` for all of them. Pod-to-pod over VXLAN, which is what workloads use, is unaffected. Check the metric's history before treating a low `Cluster health: N/8` as a regression: it read 4.5 across the whole 24h spanning the Cilium 1.19->1.20 upgrade, and it cost real time to work that out from scratch.

- **Cilium MTU**: `1450` in `cilium-config`. Don't remove — `tailscale0` at MTU 1280 would be auto-detected, breaking large downloads.
- **CoreDNS AAAA block, kept deliberately after dual-stack**: `coredns-custom` in `kube-system` returns NOERROR/empty for AAAA, so workloads resolve v4 only. It survives dual-stack for two reasons. First, it costs nothing that matters: Tailscale's direct paths do not need it, because a proxy pod advertises the GUA off its own interface and learns peer endpoints from the coordination server, and the DERP map carries v6 literals rather than hostnames. Second, removing it while `arr` denies v6 egress would make every arr connection pay a happy-eyeballs timeout — Cilium drops silently, so there is no RST to fail fast on. Lift it once the PIA VLAN has tunnelled IPv6, not before.
- **The PIA VLAN has IPv6 removed at the router, deliberately.** Pod CIDRs are cluster-wide — kube-controller-manager carves every node's `/64` out of `clusterPodNets` no matter which VLAN the node sits on — so pods on PIA workers hold a routable **GUA** even though the VLAN itself has none. With no `::/0` on those nodes that traffic dies locally with `ENETUNREACH`, fast and structurally, instead of being forwarded out the WAN around the VPN. It was previously ULA-plus-default-route, which looked safe and was not: the ULA blocked egress only because a ULA source cannot reach a global destination, and a GUA source would have sailed straight through. `deny-ipv6-egress` in `kubernetes/arr/network-policy.yaml` is the second line and stays — it is what holds if v6 is ever restored on that VLAN. PIA nodes therefore register a v4-only node IP; do not add their prefix back to `kubelet.nodeIP.validSubnets`. `kubernetes/arr/network-policy.yaml` carries a `deny-ipv6-egress` CNP — Cilium denies beat allows, so it holds regardless of the `toEntities: world` grants next to it. The excepts keep in-cluster v6 working. Do not drop it when the AAAA block eventually goes.
- **Tailscale proxy pods pin `--port=41641`**: `kubernetes/tailscale/proxy-class.yaml` injects `TS_TAILSCALED_EXTRA_ARGS` into both ProxyClasses, and `allow-tailscale-ingress` scopes `world` to that port. Without the pin tailscaled binds an ephemeral port, no policy or firewall rule can match it, and the direct path never forms — the same trap `talconfig.yaml` documents for the node-level daemon.
...
- **Who may trigger a workflow is a repo setting, and it is load-bearing.** The runners are self-hosted and the repo is public — the combination GitHub advises against. `validate-manifests.yaml` runs on `pull_request` and executes `scripts/check-flux-refs.py` *from the PR branch*, so anyone able to start a run gets code execution on a pod with `world` egress, kube-apiserver access and create-on-pods. Two settings stop that, neither visible from the code: **Settings → Actions → Policies → Restrict actors** (allowlist: `edjeffreys`, Renovate, dependabot — deny-by-default, and the one doing the real work) and **Settings → Actions → General → require approval for all outside collaborators** (kept as a backstop, because the Policies page is still Preview). Any new automation that opens PRs must be added to the allowlist or its runs are silently refused. If fork runs are ever wanted, add a second credential-free scale set rather than loosening either.
- **A Kubernetes-injected env var is not a GitHub secret, and fork PRs would get it.** GitHub withholds Actions secrets from fork pull requests, but anything the runner pod spec injects — `env`, `envFrom` — is put there by Kubernetes and is present for every job regardless. `arc-runners` carried a Terraform Cloud token, a Tailscale API key and the Packer Proxmox credentials this way, all vestigial after the `packer-*`/`replace-*-node` workflows were deleted. When a workflow is retired, check what its credentials were riding on: a GitHub secret decays harmlessly, a values-file injection does not.
...
- **Talos drains on upgrade but ignores PodDisruptionBudgets** ([siderolabs/talos#9882](https://github.com/siderolabs/talos/issues/9882)), so a PDB is not what keeps a node upgrade safe here. Nothing stops the next node going down while a Longhorn volume is still rebuilding from the last one — rebuilds run to ~90s. `scripts/upgrade-workers.sh` gates on live Longhorn and CNPG health between nodes rather than sleeping a fixed interval, which is the only thing enforcing the interval. Control planes are deliberately not in that script: different schematic, and they need an `etcd members`/`etcd alarm list` check between each.
- **Longhorn's `nodeDrainPolicy` is `always-allow`, deliberately.** The default, `block-if-contains-last-replica`, only ever blocks on the volumes created with `numberOfReplicas: 1` — the four CNPG databases and the Prometheus TSDB — where the data is either replicated at the Postgres layer or rebuildable, so the protection is redundant. It also fails opaquely: the symptom is a drain timeout (`client rate limiter Wait returned an error ... context deadline`) rather than a stated refusal, and it aborts the Talos upgrade leaving the node cordoned. Two-replica volumes only trip it when a replica is already unhealthy, which `scripts/upgrade-workers.sh` gates on between nodes. Do not "fix" this back to the default: it was changed because every node upgrade otherwise required hand-toggling the setting mid-flight.
