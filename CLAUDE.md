...
- **Longhorn *will* block a PIA node drain, and it is right to.** CNPG volumes are single-replica in Longhorn on purpose — redundancy lives at the Postgres layer via `instances: 2`, and double-replicating would multiply writes on the one physical disk. So `node-drain-policy: block-if-contains-last-replica` sees the only copy of `arr-postgres-N`'s volume and refuses to evict the node's `instance-manager`, which surfaces as a drain timeout (`client rate limiter ... would exceed context deadline`), not as a clear PDB rejection. Talos aborts the upgrade cleanly and leaves the node cordoned. Overriding with `node-drain-policy: always-allow` is safe **only** because the other CNPG instance holds a full copy — so check `readyInstances` is 2 and note which node is `currentPrimary` first, upgrade the standby's node before the primary's, and set the policy back immediately afterwards.
