...
- **Dual-stack, IPv4-primary.** Pods `10.52.0.0/16` + `2a0e:cb01:ca:fe80::/60`, services `10.53.0.0/16` + `fd53:0:0:53::/112`. The pod v6 range is a **GUA** carved from the static /56 on purpose: `enable-ipv6-masquerade` is `false`, so pods reach the internet with their own address and Tailscale proxy pods can advertise a direct endpoint instead of relaying. Turn masquerade back on and every `ts-*` pod silently falls back to a peer relay
...
- `coredns-custom` ConfigMap in `kube-system` still blocks AAAA responses, so workloads resolve v4 only despite the cluster being dual-stack. Deliberate for now — see the Key Gotchas entry
...
- **Cilium IPv6 needs ip6tables kernel modules, and Talos does not load them.** With `enable-ipv6: "true"` the agent's iptables manager fails its start hook — *"IPv6 is enabled, but the needed ip6tables tables are unavailable"* — and the whole agent crashloops, taking that node's CNI with it. `talconfig.yaml` loads `ip6_tables`, `ip6table_mangle`, `ip6table_raw` and `ip6table_filter` in the all-nodes patch for this reason. Two traps: modules load **only at boot**, so applying the config is not enough — the node must reboot; and nothing catches this ahead of time, since `talhelper validate`, `genconfig --dry-run` and `kubectl diff` all pass. It surfaces only in the agent log at startup. Kernel module lists **append** across Talos patch layers, so the workers' `iscsi_tcp` entry survives alongside these.
