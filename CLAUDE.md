...
- **Cilium IPv6 is blocked on Talos 1.13 + the tailscale extension, and the error message misdirects.** Setting `enable-ipv6: "true"` makes the agent fail its start hook with *"IPv6 is enabled, but the needed ip6tables tables are unavailable ... installing ip6tables and kernel modules"*, and the agent crashloops, taking that node's CNI with it. **Do not chase the modules.** Talos reports them `module not found` because they are compiled in, not loadable, and adding them to `machine.kernel.modules` only leaves `KernelModuleSpecController` failing on a loop. The real cause is [siderolabs/talos#13268](https://github.com/siderolabs/talos/discussions/13268): Talos 1.13 stripped tailscale from the `iptables` binaries, so the node falls back to native nft, and Cilium — which cannot do native nft before 1.20 — tries iptables-over-nft and conflicts. Every node here runs `siderolabs/tailscale`, so every node is affected. The fix is **Cilium 1.20** (reportedly with Talos 1.13.8); removing the extension or downgrading Talos are the other documented options and neither is viable here. Nothing catches it in advance — `talhelper validate`, `genconfig --dry-run` and `kubectl diff` all pass; it appears only in the agent log on the first node to restart.
